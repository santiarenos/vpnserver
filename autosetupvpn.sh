#!/bin/sh
# wget https://github.com/santiarenos/vpnserver/blob/b7e98b0f07f35173fe53c964f0286a80d3421a41/autosetupvpn
...
# - Defina sus propios valores para estas variables
# - Clave precompartida de IPsec, nombre de usuario y contraseña de VPN
# - Todos los valores DEBEN colocarse dentro de 'comillas simples'
# - NO use estos caracteres especiales dentro de los valores: \ " '

YOUR_IPSEC_PSK='i$U782lSG0%F@y*848Zt'
YOUR_USERNAME='vixion'
YOUR_PASSWORD='0Jez3MOK2&7E'
  ...
  if [ -f "$rh_file" ]; then
    ...
    if grep -q "release 7" "$rh_file"; then
      ...
      if [ "$os_type$os_ver" = "centos8" ]; then
        exiterr "CentOS Linux 8 is EOL y no soportado."
      fi
    elif grep -q "release 9" "$rh_file"; then
      os_ver=9
      grep -qi stream "$rh_file" && os_ver=9s
    else
      exiterr "Este script solo soporta CentOS/RHEL 7-9."
    fi
  elif grep -qs "Amazon Linux version 2" /etc/system-release; then
    os_type=amzn
    os_ver=2
  else
    ...
    case $os_type in
      ...
      *)
cat 1>&2 <<'EOF'
Error: Este script solo soporta los siguientes SO:
...
EOF
    ...
    if [ "$os_type" = "alpine" ]; then
      ...
      if [ "$os_ver" = 8 ] || [ "$os_ver" = "jessiesid" ]; then
        exiterr "Debian 8 o Ubuntu < 16.04 no es soportado."
      fi
    fi
  fi
}
